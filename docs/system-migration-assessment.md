...
| 荷物番号 | `nimotsu-bango/` | Cloudflare Workers + D1 + authentik OIDC | `https://nimotsu-bango.kusanaginoturugi.workers.dev/` | `https://nimotsu-bango.myouou.net/` |
...
- `nimotsu-bango`
  - Worker: `nimotsu-bango`
  - D1: `nimotsu-bango`, database_id `0aa7a802-a4e6-4014-9417-f3be7544628a`
  - Assets binding: `ASSETS`
  - public vars: `AUTHENTIK_ISSUER`, `AUTHENTIK_CLIENT_ID`
  - Secrets 再投入が必要: `AUTHENTIK_CLIENT_SECRET`, `SESSION_SECRET`
  - authentik Redirect URI: `https://nimotsu-bango.<domain>/auth/callback`
  - 権限: 一般ユーザーは自身の伝道会のみ追加・編集、管理者は全件追加・更新・無効化
- `.old/Dailytally`
...
- Cloudflare
  ...
  - DNS を Cloudflare 管理
  - Workers Paid を有効化
  - `osystem-masters`, `dailytally2`, `nimotsu-bango` を新アカウントへ再作成
...
| `nimotsu-bango.<domain>` | `nimotsu-bango` |
...
- `nimotsu-bango` は `data/records.json` から D1 投入 SQL を生成する。運用開始後に画面から追加されたデータを残す場合、全削除再投入ではなく差分投入または D1 export/import を使う。
...
| 3. Cloudflare 移行 | D1 作成/投入、Workers deploy、Pages/静的移行、cron/Browser/Secrets、独自ドメイン | 2.0-3.0 日 |
...
- 小さく始める案 A: 10-15 人日
- Lightsail 案 C: 10.5-16 人日
- 堅め案 B: 13-18 人日
- 追加で GitHub organization 移管、Rails の authentik 統合、`bulksend` 実装まで含める場合: +4-10 人日
...
- `nimotsu-bango` D1 作成、migration、records import
- `wrangler.toml` の account-specific 値を差し替え
- Secrets 再投入
- `masters.<domain>`, `dailytally2.<domain>`, `nimotsu-bango.<domain>` routes/custom domains 設定
...
- `nimotsu-bango` の現行 D1 データを export/import するか、`data/records.json` から初期再投入するか
